    ...
    if apply_full_quant:
        ...
        if dataset_name and ('aishell' in dataset_name.lower()):
            ...
            for i in range(num_samples):
                ...
                # Resolve actual OV model input names dynamically
                speech_input_name = None
                lengths_input_name = None
                for inp in ovm.inputs:
                    name = inp.get_any_name()
                    if 'speech' in name.lower() and 'length' not in name.lower():
                        speech_input_name = name
                    elif 'length' in name.lower():
                        lengths_input_name = name
                if speech_input_name is None:
                    speech_input_name = ovm.inputs[0].get_any_name()
                if lengths_input_name is None:
                    lengths_input_name = ovm.inputs[1].get_any_name() if len(ovm.inputs) > 1 else 'speech_lengths'
                calibration_samples.append({speech_input_name: speech, lengths_input_name: speech_lengths})
        else:
            raise ValueError(
...
def initialize_paraformer_support():
    """Explicitly register Paraformer with TasksManager and patch export/quantize functions.
    
    Call this from the CLI entrypoint or before exporting Paraformer models. 
    This avoids implicit import-time side effects.
    """
