...
class OVParaformerForSpeechSeq2Seq:
    ...
    def _forward_single_model(
        ...
        # Find lengths input name (might be 'speech_lengths' or 'speech_lengths.1')
        ...
        if lengths_input_name is None:
            for name in self.input_names:
                if name != speech_input_name:
                    lengths_input_name = name
                    break
        if lengths_input_name is None:
            lengths_input_name = "speech_lengths"
    ...
    def save_pretrained(
        ...
        if self._compile_only:
            raise ValueError(
                "save_pretrained() is not supported in compile_only mode. "
                "Re-load the model with compile_only=False to save."
            )
